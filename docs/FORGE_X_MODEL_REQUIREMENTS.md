...
## 29. Design Questionnaire Checkpoint: Questions 150–159

Recorded: September 2, 2026  
Status: Approved product-owner decision.

### Buyer-confirmed functional part families

- Never infer governing economic comparability from description similarity alone.
- Permit an authorized buyer to propose and confirm a functional part-family relationship between different part numbers when supported by business knowledge.
- Before confirmation, display every affected part number, description, commodity, program, vehicle position when available, and predecessor/successor context.
- Require a controlled relationship type, a concise business rationale, the applicable programs, and the effective context.
- Require the buyer to define comparison eligibility separately by analytical field or category, including piece price, material, purchased content, labor, burden, overhead, profit, conversion structure, tooling, ED&D, and other supported measures.
- Permit a relationship to support a governing comparison for selected fields while classifying other fields as directional or not comparable.
- A functional-family relationship does not convert different part numbers into an exact-part match and cannot support language claiming that they are the same part.
- Reports must distinguish exact-part evidence from buyer-confirmed functional-family evidence and clearly disclose the permitted comparison scope.
- Assign every functional part family an immutable technical identifier and retain its readable family name.
- Version membership, comparison permissions, rationale, confirming buyer, confirmation timestamp, and supporting context.
- Later corrections, removals, or scope changes create a new version and never rewrite finalized analyses that used an earlier version.
- Forge X may suggest candidate relationships using deterministic context such as selected predecessor programs, commodity, description, and vehicle position, but no candidate affects governing analysis until a buyer explicitly confirms it.

### GST take-rate validation and exception control

- Calculate implied part take rates from the official GST governing-year population and validate them before the sourcing baseline is confirmed.
- Use deterministic, version-controlled checks to identify suspicious relationships such as unexpectedly different left-hand/right-hand quantities, mutually exclusive options exceeding the supported vehicle population, missing companion parts, duplicate coverage, implausible take rates, and inconsistent program-year patterns.
- Treat detected anomalies as review indicators rather than proof that the official GST data is wrong.
- Never silently revise, rebalance, infer, or replace official GST volumes or take rates.
- Permit sourcing analysis to continue using the official GST baseline while a take-rate issue remains under review, with the limitation displayed in the consolidated buyer action list and affected outputs.
- Before finalization, require the buyer either to import or enter a controlled corrected official baseline version or to accept the exception with a business rationale.
- An accepted exception records the affected parts and years, calculated take rates, validation rule, buyer identity, rationale, timestamp, and baseline version.
- A correction creates a new immutable GST baseline version with lineage to the prior version; it never overwrites the RFQ-release baseline or a finalized analysis.
- Preserve the originally submitted GST values, validation results, buyer decision, and any corrected values as separate auditable evidence.
- Do not allow an unresolved take-rate anomaly with neither correction nor accepted exception to pass analysis finalization.

### Tooling and ED&D payment-treatment classification

- Require each supplier SFT, PST, and ED&D amount to use one of these controlled payment-treatment classifications: `Separate Lump Sum`, `Amortized in Piece Price`, `Partially Amortized`, or `Treatment Unconfirmed`.
- Preserve the supplier-submitted amount, wording, allocation, and source lineage independently from the buyer-confirmed classification.
- `Separate Lump Sum` identifies an amount paid outside piece price and includes it once in the applicable program cash-flow and NPV analysis.
- `Amortized in Piece Price` identifies an amount already embedded in piece price and prevents Forge X from adding the same amount again as a separate program cost.
- `Partially Amortized` requires the supported upfront amount and embedded per-part amount or allocation basis to be recorded separately so each component is counted once.
- `Treatment Unconfirmed` creates a buyer action and a visible limitation on affected total-program-cost, tooling, ED&D, APV, NPV, and award-comparison conclusions.
- Missing or unconfirmed payment treatment must never prevent the buyer from proceeding with the analysis.
- Forge X calculates every unaffected commercial measure and presents affected results using explicit exclusions, ranges, or an incomplete-comparison status as supported by the available evidence.
- Do not silently assume that a quoted amount is separate from or included in piece price.
- Prevent double counting through deterministic, version-controlled calculation rules and expose the treatment used in every affected output.
- A later confirmation or correction creates a new audited assumption or scenario version and never rewrites a finalized analysis.

### Company payment-treatment policy and supplier exceptions

- SFT is paid as a separate lump sum; PST and ED&D are amortized into piece price.
- Do not automatically generate alternate payment-treatment scenarios.
- Preserve a supplier submission that conflicts with company policy and call out the exception prominently.
- When PST or ED&D is submitted as a lump sum, retain the original evidence and permit a policy-aligned normalized value only when the calculation inputs are supported.
- When SFT is embedded in piece price, retain the original evidence, flag the exception, and separate it from normalized piece price only when the embedded amount or allocation is supported.
- Payment-treatment exceptions never prevent the buyer from proceeding with analysis.

### Piece-price analysis boundary

- Forge X governing analysis focuses on piece price and preservation of the supporting data.
- Preserve SFT, PST, and ED&D as separate commercial evidence, but handle PST and ED&D amortization schedules outside this analysis.
- Do not blend SFT lump sums into piece-price savings.
- For every supplier part and quote round, store the original submitted piece price, currency, price unit, supplier, part number, source package, quote round, source workbook, worksheet, cell, and submitted formula or value evidence.
- Store the buyer-confirmed active piece-price record separately from the immutable submitted observation.
- Store any Forge-calculated comparable or corrected price separately with its reason, rule version, inputs, and lineage.
- Every analysis identifies the exact active price record used and never overwrites the submitted price.

### Corporate knowledge levels

- Separate `Observed Evidence`, `Confirmed Knowledge`, and `Corporate Standard` in the data model.
- Observed Evidence preserves exactly what appeared in a specific source and is immutable.
- Confirmed Knowledge is reusable only within its authorized supplier, plant, commodity, process, part-family, region, and effective-period scope.
- Corporate Standards are master-authorized rules, taxonomies, mappings, or benchmarks intended for broader company use.
- Repeated observations may create a candidate suggestion but never automatically become confirmed knowledge or a corporate standard.
- Every confirmed knowledge item retains its supporting evidence, scope, effective dates, confirmer or approver, and supersession history.

### Knowledge scope and precedence

- Resolve applicable knowledge from the most specific supported scope to the broadest: exact supplier plant and exact part; supplier plant and confirmed part family or process; supplier plant and commodity; supplier corporate family, region, and commodity; regional commodity; then company-wide corporate standard.
- A more specific record does not delete, modify, or replace the broader record.
- Apply a record only when its scope, effective dates, units, currency, and evidence requirements match the analysis context.
- Display the applied knowledge level and provide access to its supporting evidence.
- When equally specific active records conflict, flag the conflict for resolution and never select one silently.

## 30. Design Questionnaire Checkpoint: Questions 160–169
...
- Every normalized database record retains a direct immutable link to its original source observation.
- Lineage resolves to the applicable import event, source workbook, worksheet, cell or range, submitted value or formula, supplier, part, and quote-round context.
- Derived results retain links to every source and intermediate record used by the calculation.

### Immutable supplier activity history

- Never rewrite an existing supplier observation, activity, interpretation, confirmation, or calculated result.
- Corrections, additions, removals, supersessions, and changed commercial positions create new versioned records linked to the records they supersede.
- Preserve all supplier activity so Forge X can develop evidence-supported supplier behavior profiles over time.
- Store each supplier change with immutable before-and-after evidence and a controlled activity type, including price change, formula change, scope change, omission, correction, addition, removal, and reaffirmation.

### Rebuildable profile summaries

- Use precomputed supplier-profile summary tables for responsive searches, dashboards, and drill-down entry points.
- Detailed immutable evidence remains authoritative; every summary is derived and rebuildable.
- Every summary stores its calculation version, evidence cutoff date, eligibility-rule version, and last successful rebuild identifier.
- Rebuild summaries atomically so users see either the complete prior version or the complete replacement, never a partial refresh.

### Scoped supplier profiles

- Calculate supplier profiles separately by supplier code, plant, commodity, and region before producing any corporate-family rollup.
- Corporate-family results retain the contributing scoped profiles and cannot hide conflicting plant- or commodity-level behavior.

### Bitemporal corporate knowledge

- Every confirmed knowledge record stores both its business-valid period and its Forge X recording period.
- Distinguish when a fact applied in the business from when Forge X learned, confirmed, corrected, or superseded it.
- Late-arriving evidence never changes the recorded history of what was known at an earlier analysis date.

### Integrity constraints and identifiers

- Enforce foreign keys, required relationships, controlled-value constraints, precision rules, and state-transition constraints in the database in addition to application validation.
- Do not use cascading deletion for immutable evidence, knowledge, audit, analysis, or supplier-activity records.
- Assign application-generated immutable technical identifiers independently from editable business labels and external identifiers.
- Prefer time-orderable, collision-resistant identifiers when supported by the approved database stack, while preserving source package numbers, supplier codes, part numbers, and other business keys separately.

## 31. Design Questionnaire Checkpoint: Questions 170–179

Recorded: September 2, 2026  
Status: Approved product-owner decisions.

### Normalized authoritative core

- Maintain a normalized transactional core for authoritative identities, observations, versions, relationships, and audit history.
- Keep denormalized search, dashboard, and profile projections separate and rebuildable from the normalized core.

### Complete source preservation

- Retain exact submitted representations, recognized formulas, normalized values, fingerprints, extraction diagnostics, and source-location metadata needed to reproduce and defend every stored observation.
- Do not make a derived search index, cached value, or summary the sole copy of business evidence.

### Controlled schema evolution

- Apply versioned, forward-only, transactional schema migrations with preflight validation, recoverable checkpoints, and post-migration integrity verification.
- Never modify an immutable business record merely to accommodate a schema change.

### No silent merging or deletion

- Never automatically merge suspected duplicate suppliers, parts, plants, observations, knowledge records, or business relationships.
- Fingerprints and matching rules may identify candidates, but an authorized confirmation governs any business relationship or duplicate disposition.
- Do not physically delete historical business evidence through normal application workflows; use explicit status, supersession, and analytical-eligibility controls.

### Fingerprints and integrity verification

- Store layered fingerprints for source files, worksheets, logical observations, extracted structures, and normalized content.
- Verify fingerprints during import, publication, restore, migration, reconciliation, and evidence reproduction.

### Query-driven indexing

- Design indexes from approved query patterns and validate them against representative full-scale datasets.
- Cover common supplier, part, commodity, plant, program, source-package, quote-round, activity-history, effective-date, and evidence-lineage access paths.
- Record query plans and latency benchmarks, and revise indexes through versioned schema migrations rather than uncontrolled production changes.

### Search and summary projections

- Permit full-text, faceted-search, and analytical projections for speed, but keep them derived, versioned, and rebuildable.
- Atomically publish projection generations and retain the governing evidence cutoff and build manifest.

### Automated intelligence boundary

- Automated or AI-assisted methods may suggest mappings, classifications, relationships, and knowledge candidates but cannot silently create governing corporate knowledge or alter immutable evidence.
- Governing use requires the approved deterministic rule or authorized human confirmation appropriate to the knowledge level.

### Recovery readiness

- Create encrypted, integrity-checked database checkpoints and publication packages with signed manifests and complete schema, rule, and evidence-version metadata.
- Test restoration, migration, summary rebuilding, audit-chain verification, and analytical reproduction on a defined recurring schedule.
- A backup is not considered valid until an automated restore and integrity test succeeds.
